	...
	view = {
		...
		makeDOMNodeIcon: function(html, setting, node) {
			...
			html.push("<span id='", node.tId, consts.id.ICON,
				...
				"' class='", view.makeNodeNameClass(setting, node), "'>",name,"</span>");
		},
		...
		makeNodeNameClass: function(setting, node) {
			var icoCss = "name";
			if (node.viewtype && node.viewtype=='2') {
				icoCss+=" noUse";
			}else if(node.viewtype && node.viewtype=='3') {
				icoCss+=" noConfirm";
			}
			return icoCss;
		},
		...
		setNodeNameClass: function(setting, treeNode) {
			var spanObj = $$(treeNode, consts.id.SPAN, setting);
			var nameClass = view.makeNodeNameClass(setting, treeNode);
			if (nameClass) {
				spanObj.addClass(nameClass);
			}
		},
			...
			var zTreeTools = {
				...
				updateNode : function(node, checkTypeFlag) {
					...
					if (nObj.get(0) && tools.uCanDo(setting)) {
						...
						view.setNodeNameClass(setting, node);
					}
				}
			}
