...
public final class StringHelper {
	...
	public static String[] multiply(String string, Iterator<Object> placeholders, Iterator<Object> replacements) {
	...
	public static String getString(Object... objs) {
		if (objs != null && objs.length > 0) {
			...
			for (int i = 0; i < objs.length; i++) {
				sb.append(objs[i] == null ? EMPYT_STR : objs[i].toString());
			}
			return sb.toString();
		}
		return StringHelper.EMPYT_STR;
	}

	
	public static String [] insertElementBehind (String [] strArr,String... ele) {
		String destination[]=new String[strArr.length+ele.length];
		System.arraycopy(strArr, 0, destination, 0, strArr.length);
		if (ele != null && ele.length>0) {
			for(int i = strArr.length; i < destination.length; i++) {
				destination[i] = ele[i-strArr.length];
			}
		}
		return destination;
	}
